...
export async function recordAction(
  ...
  const r = await dbRun(
    `INSERT INTO admin_actions (timestamp, username, action, description, undo_data)
     VALUES (?, ?, ?, ?, ?)`,
    [new Date().toISOString(), username, action, description, undoData ? JSON.stringify(undoData) : null],
  );
  return r.lastID;
}
...
export async function adminActionsRoutes(app: FastifyInstance) {
  ...
  app.post("/admin/actions/:id/undo", async (request, reply) => {
    ...
    const action = await dbGet<ActionRow>(
      `SELECT * FROM admin_actions WHERE id = ? AND undone = 0`, [parseInt(id, 10)]
    );
    if (!action) return reply.code(404).send({ error: "Action not found or already undone" });

    const data = action.undo_data ? JSON.parse(action.undo_data) : null;

    try {
      await performUndo(action.action, data);
      await dbRun(`UPDATE admin_actions SET undone = 1 WHERE id = ?`, [action.id]);
      return reply.send({ ok: true, undone: action.action });
    } catch (err) {
      return reply.code(500).send({ error: `Undo failed: ${(err as Error).message}` });
    }
  });
...
async function performUndo(action: string, data: Record<string, unknown> | null): Promise<void> {
  ...
  switch (action) {
    case "delete-db": {
      ...
      await dbRun(
        `INSERT INTO files (id, filename, original_name, path, size, mime_type, user_id, created_at, is_public, storage_backend)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [data.id, data.filename, data.original_name, data.path, data.size,
         data.mime_type, data.user_id, data.created_at, data.is_public, data.storage_backend]
      );
      break;
    }
    case "db-delete": {
      ...
      await dbRun(`INSERT INTO "${table}" (${columns.map((c) => `"${c}"`).join(", ")}) VALUES (${placeholders})`, values);
      break;
    }
    case "user-create": {
      if (data.userId) {
    ...
    case "migrate": {
      ...
      if (data.fromPath && data.toPath) {
        ...
        if (!fs.existsSync(src)) throw new Error("Migrated file no longer exists at destination");
        const destDir = path.dirname(dest);
        if (!fs.existsSync(destDir)) fs.mkdirSync(destDir, { recursive: true });
        fs.renameSync(src, dest);
        await dbRun(`UPDATE files SET path = ?, user_id = ? WHERE path = ?`,
          [dest, data.fromUserId, src]);
      }
      break;
    }
    default:
