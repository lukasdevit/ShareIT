...
import { getLogs, clearLogs, readLogFile, writeLog } from "../../services/logService.js";
...
export async function adminLogRoutes(app: FastifyInstance) {
  ...
  app.get("/admin/logs", async (request, reply) => {
    ...
    const parsedLines = Math.min(Math.max(parseInt(lines || "200", 10) || 200, 1), 2000);
    const parsedLevel = Math.min(Math.max(parseInt(level || "30", 10) || 30, 10), 60);
  ...
  app.delete("/admin/logs", async (request, reply) => {
    ...
    if (request.user?.username) {
      await recordAction(request.user!.username, "logs-clear", "Cleared server logs");
    }
    writeLog({
