...
export async function adminBackupManageRoutes(app: FastifyInstance) {
  ...
  app.post("/admin/backup/upload", async (request, reply) => {
    ...
    if (ext !== ".db") return reply.code(400).send({ error: "Only .db files are accepted" });
    ...
    try {
      ...
      try {
        execSync(`sqlite3 "${destPath}" "PRAGMA integrity_check;"`, { stdio: "pipe", timeout: 5000 });
      } catch {
        fs.unlinkSync(destPath);
        return reply.code(400).send({ error: "Uploaded file is not a valid SQLite database" });
      }

      const stat = fs.statSync(destPath);
      if (request.user?.username) {
        await recordAction(request.user!.username, "backup-upload", `Uploaded backup: ${destName}`, { filename: destName });
      }
      return reply.send({
        ok: true,
        backup: { filename: destName, size: stat.size, created: stat.birthtime.toISOString() },
      });
    } catch (err) {
  ...
  app.delete("/admin/backup/delete", {
    ...
    if (request.user?.username) {
      await recordAction(request.user!.username, "backup-delete", `Deleted backup: ${safe}`, { filename: safe });
    }
  ...
  app.post("/admin/backup/restore", {
    ...
    if (confirm !== "RESTORE") {
      return reply.code(400).send({ error: "Type 'RESTORE' in the confirm field to proceed" });
    }
    ...
    try {
      execSync(`sqlite3 "${srcPath}" "PRAGMA integrity_check;"`, { stdio: "pipe", timeout: 10000 });
    } catch {
      return reply.code(400).send({ error: "Backup file is corrupted (integrity check failed)" });
    }
    ...
    try {
      ...
      return reply.code(500).send({ error: `Failed to create safety backup: ${(err as Error).message}` });
    }

    // Perform restore using sqlite3 .backup command (transactional)
    try {
      execSync(`sqlite3 "${DB_PATH}" ".restore '${srcPath}'"`, { stdio: "pipe", timeout: 30000 });
    } catch (err) {
      // Restore safety backup on failure
      try { fs.copyFileSync(safetyBackup, DB_PATH); } catch { /* best effort */ }
    ...
    if (request.user) {
      recordAction(request.user!.username, "backup-restore", `Restored database from: ${filename}`, {
        filename, safetyBackup: path.basename(safetyBackup),
      }).catch(() => {});
    }
    ...
    return reply.send({
      ...
      message: "Database restored successfully. The server may need a restart for in-memory caches to refresh.",
    });
  });
}
