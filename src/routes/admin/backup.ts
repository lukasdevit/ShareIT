...
export async function adminBackupRoutes(app: FastifyInstance) {
  ...
  app.get("/admin/backup/latest", async (_request, reply) => {
    try {
      ...
      const files = fs.readdirSync(path.join(process.cwd(), "uploads", "backups"))
  ...
  app.get("/admin/backup/history", async (_request, reply) => {
    const rows = await dbAll<{
      id: number; timestamp: string; destination: string;
      status: string; size_bytes: number | null; error: string | null;
