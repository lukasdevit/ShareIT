...
function getAllDiskFiles(dir: string, baseDir: string, files: Map<string, string>): void {
  if (!fs.existsSync(dir)) return;
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const full = path.join(dir, entry.name);
    if (entry.isDirectory()) { getAllDiskFiles(full, baseDir, files); }
    else { files.set(path.relative(baseDir, full), full); }
  }
}
...
function cleanEmptyDirs(absPath: string): void {
  ...
  while (dir !== UPLOAD_DIR && dir !== path.join(UPLOAD_DIR, "share")) {
    try { if (fs.readdirSync(dir).length === 0) fs.rmdirSync(dir); else break; }
    catch { break; }
...
async function resolveIssue(checkId: string, issueId: number, action: string, username?: string): Promise<void> {
  const issue = await dbGet<{ type: string; file_id: number | null; disk_path: string | null; resolved: number }>(
  ...
  if (action === "delete-db" && issue.file_id) {
    ...
    const row = await dbGet<Record<string, unknown>>(
      `SELECT * FROM files WHERE id = ?`, [issue.file_id]
    );
    ...
    if (fs.existsSync(absPath)) { fs.unlinkSync(absPath); cleanEmptyDirs(absPath); }
    if (username) {
      await recordAction(username, "delete-file", `Deleted file: ${issue.disk_path}`, { diskPath: issue.disk_path });
    }
  }

  await dbRun(`UPDATE integrity_issues SET resolved = 1, action_taken = ? WHERE id = ?`, [action, issueId]);
}

export async function adminIntegrityRoutes(app: FastifyInstance) {
  // List saved checks
  app.get("/admin/storage/integrity", async (_request, reply) => {
    const checks = await dbAll<CheckRow>(`SELECT * FROM integrity_checks ORDER BY created_at DESC LIMIT 20`);
    return reply.send({ checks });
  });

  // Start new check
  app.post("/admin/storage/integrity", {
    schema: {
      body: {
    ...
    if (body.username && !userId) {
      const user = await dbGet<{ id: number }>(`SELECT id FROM users WHERE username = ?`, [body.username]);
    ...
    const dbFiles = await dbAll<{
      id: number; filename: string; original_name: string; path: string; size: number; user_id: number | null;
    ...
    const scanDir = userId ? path.join(UPLOAD_DIR, "share", String(userId)) : path.join(UPLOAD_DIR, "share");
    ...
    await dbRun(
      `INSERT INTO integrity_checks (check_id, created_at, total_issues, missing_files, orphaned_files, size_mismatches)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [checkId, now, insertRows.length, missingFiles, orphanedFiles, sizeMismatches]
    );
    ...
    return reply.send({ checkId, total: insertRows.length, summary: { missingFiles, orphanedFiles, sizeMismatches } });
  });
  ...
  app.get("/admin/storage/integrity/:checkId", async (request, reply) => {
    ...
    const { offset, limit, type, userId, username } = request.query as {
      offset?: string; limit?: string; type?: string; userId?: string; username?: string;
    };

    const check = await dbGet<CheckRow>(`SELECT * FROM integrity_checks WHERE check_id = ?`, [checkId]);
    ...
    if (resolvedUserId !== null) {
      conditions.push("user_id = ?"); params.push(resolvedUserId);
    }

    const where = conditions.join(" AND ");

    const [totalRow, issues, unresolvedRow] = await Promise.all([
      dbGet<{ cnt: number }>(`SELECT COUNT(*) AS cnt FROM integrity_issues WHERE ${where}`, params),
      ...
      dbGet<{ cnt: number }>(
        `SELECT COUNT(*) AS cnt FROM integrity_issues WHERE ${where} AND resolved = 0`, params
      ),
    ]);
  ...
  // Resolve single
  app.post("/admin/storage/integrity/:checkId/resolve", {
    schema: {
      body: {
        type: "object" as const, required: ["issueId", "action"],
        properties: {
          issueId: { type: "number" },
  ...
  // Resolve bulk
  app.post("/admin/storage/integrity/:checkId/resolve-bulk", {
    schema: {
      body: {
        type: "object" as const, required: ["issueIds", "action"],
        properties: {
    ...
    for (const id of issueIds) {
      try { await resolveIssue(checkId, id, action, request.user?.username); resolved++; } catch { /* skip */ }
    }
    return reply.send({ ok: true, resolved });
  });

  // Import orphaned files into DB
  app.post("/admin/storage/integrity/:checkId/import", {
    schema: {
      body: {
        type: "object" as const, required: ["issueIds"],
        properties: {
          issueIds: { type: "array", items: { type: "number" } },
          userId: { type: "number" },
    ...
    const { checkId } = request.params as { checkId: string };
    const { issueIds, userId, originalName } = request.body as { issueIds: number[]; userId?: number; originalName?: string };
    ...
    for (const issueId of issueIds) {
      ...
      const result = await dbRun(
        `INSERT INTO files (filename, original_name, path, size, mime_type, user_id, created_at, is_public, storage_backend)
         VALUES (?, ?, ?, ?, ?, ?, ?, 1, 'local')`,
        [filename, nameForDb, absPath, stat.size, getMimeType(diskName), resolvedUserId ?? null, now]
      );
      await dbRun(`UPDATE integrity_issues SET resolved = 1, action_taken = 'imported' WHERE id = ?`, [issueId]);
      imported.push({ issueId, fileId: result.lastID });
      if (request.user?.username) {
        await recordAction(request.user!.username, "import", `Imported orphaned file: ${diskName} (new ID #${result.lastID})`, {
          fileId: result.lastID, filename, originalName: nameForDb,
        });
      }
    }
  ...
  app.post("/admin/storage/migrate", {
    schema: {
      body: {
        type: "object" as const, required: ["paths", "toUserId"],
        properties: {
    ...
    const { paths, toUserId } = request.body as { paths: string[]; toUserId: number };
    ...
    for (const relPath of paths) {
      try {
        ...
        if (!fs.existsSync(absSrc)) { errors.push(`${relPath}: file not found`); continue; }

        const fromUserId = extractUserIdFromPath(relPath);
        if (fromUserId === null) { errors.push(`${relPath}: could not determine source user`); continue; }
        if (fromUserId === toUserId) { errors.push(`${relPath}: already belongs to user ${toUserId}`); continue; }
        ...
        await dbRun(
          ...
          [absDest, toUserId, absSrc]
        );

        migrated.push(`${relPath} → ${destRel}`);
        if (request.user?.username) {
          await recordAction(request.user!.username, "migrate", `Migrated file: ${relPath} → user #${toUserId}`, {
