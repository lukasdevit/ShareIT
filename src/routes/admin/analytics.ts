...
export async function adminAnalyticsRoutes(app: FastifyInstance) {
  app.get("/admin/analytics", { config: { rateLimit: { max: 20, timeWindow: 60000 } } }, async (_request, reply) => {
    ...
    const [usersRow, filesRow, todayRow, dailyRows, topRows, catRows] = await Promise.all([
      dbGet<{ users: number }>(`SELECT COUNT(*) AS users FROM users`),
      dbGet<{ files: number; bytes: number }>(`SELECT COUNT(*) AS files, COALESCE(SUM(size), 0) AS bytes FROM files`),
      dbGet<{ today: number }>(`SELECT COUNT(*) AS today FROM files WHERE created_at >= ?`, [today]),
      dbAll<DailyPoint>(`SELECT DATE(created_at) AS day, COUNT(*) AS count, COALESCE(SUM(size), 0) AS bytes FROM files WHERE created_at >= ? GROUP BY day ORDER BY day`, [monthAgo]),
      dbAll<TopUser>(`SELECT u.username, COUNT(f.id) AS files, COALESCE(SUM(f.size), 0) AS bytes FROM users u LEFT JOIN files f ON f.user_id = u.id GROUP BY u.id ORDER BY bytes DESC LIMIT 10`),
      dbAll<Category>(`SELECT CASE WHEN mime_type LIKE 'image/%' THEN 'Images' WHEN mime_type LIKE 'video/%' THEN 'Videos' WHEN mime_type LIKE 'text/%' OR mime_type IN ('application/json','application/xml','application/javascript') THEN 'Text / Code' ELSE 'Other' END AS category, COUNT(*) AS count, COALESCE(SUM(size), 0) AS bytes FROM files GROUP BY category ORDER BY bytes DESC`),
    ]);
