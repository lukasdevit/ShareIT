...
function UploadsChart({ daily, maxCount }: { daily: any[]; maxCount: number }) {
  return (
...
function TopUsers({ users, maxBytes }: { users: any[]; maxBytes: number }) {
  const active = users.filter((u: any) => u.bytes > 0);
  return (
    <section className="card">
            ...
            <div key={u.username} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between text-xs mb-0.5">
                  ...
                  <span className="text-zinc-500 ml-2 whitespace-nowrap">{formatSize(u.bytes)}</span>
                </div>
                <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div className="h-full bg-blue-500 rounded-full" style={{ width: `${(u.bytes / maxBytes) * 100}%` }} />
                </div>
              </div>
            </div>
          ))}
...
function FileTypes({ categories, maxBytes }: { categories: any[]; maxBytes: number }) {
  return (
    <section className="card">
          ...
          {categories.map((c: any) => (
            <div key={c.category}>
              <div className="flex justify-between text-xs mb-0.5">
                ...
                <span className="text-zinc-500">{c.count} files · {formatSize(c.bytes)}</span>
              </div>
              <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
