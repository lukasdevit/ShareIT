...
export function AdminActions({ apiFetch }: Props) {
  ...
  async function handleUndo(id: number, action: string) {
    const unUndoable = ["delete-file", "backup-restore", "backup-delete", "backup-upload", "storage-config", "logs-clear", "user-edit", "user-delete"];
  ...
  return (
    <section className="card space-y-4">
      <div className="flex items-center justify-between">
        ...
        {undoneCount > 0 && (
          <button type="button" onClick={clearUndone} disabled={clearing}
      ...
      <p className="text-xs text-zinc-500">
        Log of all admin operations. Click <span className="text-amber-400">Undo</span> to reverse an action where possible.
      </p>

      {loading ? (
        <RowSkeleton cols={5} rows={5} />
      ) : actions.length === 0 ? (
        <EmptyState icon="🕓" title="No actions recorded" description="Admin actions will appear here as you use the admin panel." />
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-zinc-400">
            ...
            <tbody>
              {actions.map((a) => (
                <tr key={a.id} className={`border-b border-zinc-800/50 ${a.undone ? "opacity-40" : ""}`}>
                  <td className="py-1.5 pr-3 text-zinc-500">{new Date(a.timestamp).toLocaleString()}</td>
                  <td className="py-1.5 pr-3 text-zinc-300">@{a.username}</td>
                  <td className="py-1.5 pr-3">
                    <span className={
                      a.action.includes("delete") || a.action === "db-delete" ? "text-red-400"
                        : a.action.includes("import") || a.action.includes("create") || a.action === "backup-upload" ? "text-green-400"
                          : a.action.includes("migrate") || a.action.includes("edit") ? "text-blue-400"
                            : a.action.includes("restore") || a.action.includes("backup-run") ? "text-amber-400"
                              : "text-zinc-400"
                    }>
                      {ACTION_LABELS[a.action] || a.action}
                    </span>
                  </td>
                  <td className="py-1.5 pr-3 text-zinc-500 max-w-[300px] truncate">{a.description}</td>
                  <td className="py-1.5">
                    {a.undone ? (
                      <span className="text-zinc-600 text-[10px]">Undone</span>
                    ) : ["delete-file", "backup-restore", "backup-delete", "backup-upload", "storage-config", "logs-clear", "user-edit", "user-delete"].includes(a.action) ? (
                      <span className="text-zinc-600 text-[10px]" title="Not undoable">—</span>
                    ) : (
                      <button
                        type="button"
